...
namespace Balancer_Math:
    ...
    func get_spot_price{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        ...
        let (local x : Uint256) = SafeUint256.mul(a_balance, a_weight.n)
        let (local y : Uint256) = SafeUint256.mul(b_balance, b_weight.n)
        let fee_adj : Uint256 = SafeUint256.sub_le(fee.d, fee.n)

        local num_ratio : Ratio = Ratio(n=x, d=a_weight.d)
        local den_ratio : Ratio = Ratio(n=y, d=b_weight.d)

        let (local z : Uint256) = SafeUint256.mul(num_ratio.n, den_ratio.d)
        let (local z_fee : Uint256) = SafeUint256.mul(z, fee.d)
        let (local k : Uint256) = SafeUint256.mul(den_ratio.n, num_ratio.d)
        let (local k_fee : Uint256) = SafeUint256.mul(fee_adj, k)
    ...
    func get_pool_minted_given_single_in{
        ...
        let (local i : Uint256) = SafeUint256.sub_le(step_one_times_fee.d, step_one_times_fee.n)
    ...
    func get_single_out_given_pool_in{
        ...
        let x : Uint256 = SafeUint256.sub_le(exit_fee.d, exit_fee.n)
        ...
        let (local y : Uint256) = SafeUint256.mul(supply, times_pool_in.d)
        let y_adj : Uint256 = SafeUint256.sub_le(y, times_pool_in.n)
        ...
        let (local z : Uint256) = SafeUint256.mul(a_balance, times_balance_out.d)
        let z_adj : Uint256 = SafeUint256.sub_le(z, times_balance_out.n)
        ...
        let k : Uint256 = SafeUint256.sub_le(step_one_times_fee.d, step_one_times_fee.n)
    ...
    func get_out_given_in{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
        ...
        local one_ratio : Ratio = Ratio(Uint256(1, 0), Uint256(1, 0))

        let x : Uint256 = SafeUint256.sub_le(swap_fee.d, swap_fee.n)
        ...
        let (local one_minus_all : Ratio) = ratio_diff(one_ratio, take_root_of_exponent)
    ...
    func _get_balance_needed_from_pool_supply{
        ...
        let (local amount_required : Uint256, _) = SafeUint256.div_rem(
